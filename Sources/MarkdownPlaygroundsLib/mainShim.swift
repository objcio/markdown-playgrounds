...
@objc class D: NSObject, NSTextStorageDelegate {
    var willProcess: (_ range: NSRange) -> () = { _ in () }
    func textStorage(_ textStorage: NSTextStorage, willProcessEditing editedMask: NSTextStorageEditActions, range editedRange: NSRange, changeInLength delta: Int) {
        willProcess(editedRange)
    }
...
//        editor.layoutManager?.replaceTextStorage(TextStorage(attributedString: editor.textStorage ?? NSAttributedString()))
        editor.textStorage?.delegate = d
        
        self.view = splitView([editorScrollView, outputScrollView])
    }
    
    var invalidRange: NSRange? = nil
...
//            dump(note)
//            measure(name: "did change") {
//                print("Going to highlight")
//                self.highlight()
//                print(self.editor.selectedRange())
//            }
        }
    ...
    func highlight(from range: NSRange? = nil) {
        ...
        do {
            ...
            let zipped = try measure(name: "\(filtered.count) items") { zip(filtered, try swiftHighlighter.highlight(filtered.map { $0.text })) }
